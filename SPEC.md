...
WSL2 の cron はターミナルを閉じると停止するため、Windows タスクスケジューラから起動する。
...
`run.sh` は venv を有効化して `run.py fetch && claude -p ... && run.py apply` を実行し、非ゼロ終了時は `runs.jsonl` にエラーを記録する。

初回は 1日1回（朝）で運用し、溜まり具合を見て頻度を調整する。
